...
// Konfigurasi koneksi ke database sumber (Santri)
    ...
    sourceDB.query('SELECT * FROM santri', (err, results) => {
        ...
        const dataToInsert = results.map((row) => [row.nama, row.hp, row.email, row.gender, row.alamat, row.angkatan, row.jurusan, row.minat, row.status]);
        targetDB.query('INSERT INTO Santri (nama, hp, email, gender, alamat, angkatan, jurusan, minat, status) VALUES ?', [dataToInsert], (err) => {
            if (err) {
